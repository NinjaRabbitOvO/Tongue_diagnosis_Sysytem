...
from unet_keras.tongue_diagnosis_design.tongue_diagnosis_tai_color.tai_color2_classify_model import resnet50


def main():
    data_root = os.path.abspath(os.path.join(os.getcwd(), "./."))  # get FundusVessels root path
    image_path = os.path.join(data_root, "data_set", "train_tai_color")  # flower FundusVessels set path
    ...
    with open('class_indices_tai_color.json', 'w') as json_file:
    ...
    feature = resnet50(num_classes=4, include_top=False)
    ...
    for epoch in range(epochs):
        ...
        if val_loss.result() < best_test_loss:
            ...
            model.save_weights("./save_weights_tai_color/resNet_50.ckpt", save_format="tf")
