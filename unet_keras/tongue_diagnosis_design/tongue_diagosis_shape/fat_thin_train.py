...
from unet_keras.tongue_diagnosis_design.tongue_diagosis_shape.model import resnet50


def main():
    data_root = os.path.abspath(os.path.join(os.getcwd(), "./."))  # get FundusVessels root path
    image_path = os.path.join(data_root, "data_set", "tongue_shape_data")  # flower FundusVessels set path
    train_dir = os.path.join(image_path, "train2_fat_thin")
    validation_dir = os.path.join(image_path, "val2_fat_thin")
    ...
    with open('class_indices_fat_thin.json', 'w') as json_file:
    ...
    for epoch in range(epochs):
        ...
        if val_loss.result() < best_test_loss:
            ...
            model.save_weights("./save_weights_fat_thin/resNet_50.ckpt", save_format="tf")
