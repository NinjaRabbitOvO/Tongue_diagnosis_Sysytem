...
def read_image_filenames(data_dir):
    intented_tongue_dir = data_dir + 'intented_tongue/'
    normal_tongue_dir = data_dir + 'normal_tongue/'

    #构建特征数据集，值为对应的图片文件名
    intented_tongue_filenames = tf.constant([intented_tongue_dir + fn for fn in os.listdir(intented_tongue_dir)])
    #齿痕舌 0-55    0
    normal_tongue_filenames = tf.constant([normal_tongue_dir + fn for fn in os.listdir(normal_tongue_dir)])
    #正常舌 0-76    1

    filenames = tf.concat([intented_tongue_filenames,normal_tongue_filenames],axis=-1)

    labels = tf.concat([
        tf.zeros(intented_tongue_filenames.shape,dtype=tf.int32),
...
def test_image_predict(path,start,finish,image_size=(224,224)):
    ...
    for i in range(0,finish-start):
        if np.argmax(preds[i]) == 0:
            label = "intented_tongue " + str(preds[i][0])
...
test_image_predict(test_data_dir,0,9)