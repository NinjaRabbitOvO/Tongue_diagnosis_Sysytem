...
from unet_keras.yolov4_pytorch_master.utils.utils import bbox_iou, merge_bboxes
...
class YOLOLoss(nn.Module):
    def __init__(self, anchors, num_classes, img_size, label_smooth=0, cuda=False, normalize=True):
    ...
    def forward(self, input, targets=None):
        ...
        print("mask")
        print(mask.shape)  #(1,3,13,13)
        print("noobj_mask")
        print(noobj_mask.shape)#(1,3,13,13)
        print("t_box")
        print(t_box.shape) #(1,3,13,13,4)
        #---------------------------------------------------------------#
        ...
        print("pred_boxes_for_ciou:")
        print(pred_boxes_for_ciou.shape)  #(1,3,13,13,4)  说明这个时候得到的是一个框框

        print("测试置信度loss值是几个特征层来计算的")  #三个
    ...
    def get_ignore(self,prediction,target,scaled_anchors,in_w, in_h,noobj_mask):
        ...
        for i in range(bs):
            ...
            print("get_ignore中的pre_boxes")
            #每一个特征层都有一个torch.Size([3, 26, 26, 4])
            print(pred_boxes_for_ignore.shape)
            #-------------------------------------------------------#
            #   将预测结果转换一个形式
            ...
            if len(target[i]) > 0:
                ...
                print("此时的真实框与预测框最大的IOU的值：")
                print(anch_ious_max.shape)
                #其实也是计算的每一个特征层上谕CIOU的值
                #print(anch_ious_max)
